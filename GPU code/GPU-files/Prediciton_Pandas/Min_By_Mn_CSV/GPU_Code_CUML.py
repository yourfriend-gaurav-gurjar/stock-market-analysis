...
import cuml.experimental.preprocessing
...
def prepare_data(series, n_test, n_lag, n_seq):
    ...
    scaler = cuml.experimental.preprocessing.MinMaxScaler(feature_range=(-1, 1))
...
def forecast_lstm(model, X, n_batch):
    ...
    X_f = np.asarray(X_f).astype(np.float32)
    #print(X_f)
    #print(len(X_f))
    # make forecast
...
def mean_absolute_percentage_error(y_true, y_pred):
    y_true, y_pred = cupy.array(y_true), cupy.array(y_pred)
    y_true.get(), y_pred.get()

    return cupy.mean(cupy.abs((y_true - y_pred) / y_true)) * 100
...
def evaluate_forecasts(test, forecasts, n_lag, n_seq, metric="rmse"):
    ...
    for i in range(n_seq):
        ...
        print(type(actual))
        actual = cupy.array(actual)
        print(type(actual))
        predicted = [forecast[i] for forecast in forecasts]
        print(type(predicted))
        predicted= cupy.array(predicted)
        print(type(predicted))
        if metric.lower() == "rmse":
            err = cupy.sqrt(cuml.metrics.regression.mean_squared_error(actual, predicted))
