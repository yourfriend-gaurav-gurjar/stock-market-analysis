    ...
    try:
        if len(holiday1) != 0:
            ...
            def build_lstm_network(X_seq_length,
                ...
                # design network
                model = Sequential()
                model.add(LSTM(units=num_cell_units,
                               batch_input_shape=(num_sequences_per_batch, X_seq_length, X_feature_length),
                               stateful=keep_state))
            ...
            if use_stateful_model:
                ...
                for i in range(NUM_EPOCHS_TRAINING):
                    ...
                    training_history.append(
                        multistep_model_close.fit(X_train_close, y_train_close, epochs=1, batch_size=1, verbose=1,
                                                  shuffle=False))
                    multistep_model_close.reset_states()
            else:
                training_history = multistep_model_close.fit(X_train_close, y_train_close, epochs=NUM_EPOCHS_TRAINING,
                                                             batch_size=0, validation_split=0.25, verbose=1,
                                                             shuffle=False)
            ...
            def forecast_lstm(model, X, n_batch):
                ...
                if X_feature_length is None:
                    ...
                    raise ValueError(
                        "ERROR:: vector length for input is not equal to look back length of {}".format(look_back))
            ...
            prediction_start_dates = np.reshape(prediction_start_dates, (prediction_start_dates.shape[0], 1)).astype(
                np.float64)
            price_on_start_dates = dataset_close[num_train_start_points:]["Close"].values
            price_on_start_dates = np.reshape(price_on_start_dates, (price_on_start_dates.shape[0], 1)).astype(
                np.float64)
            ...
            actual_price_start_date_plus_five = np.reshape(actual_price_start_date_plus_five,
                                                           (actual_price_start_date_plus_five.shape[0], 1))
            ...
            for index, row in df_forecast_five_day.iterrows():
                up = ((row["Actual_close_plus_5"] - row["Actual_close"] >= 0) and (
                    (row["Pred_close_plus_5"] - row["Actual_close"] >= 0)))
            down = ((row["Actual_close_plus_5"] - row["Actual_close"] <= 0) and (
                (row["Pred_close_plus_5"] - row["Actual_close"] <= 0)))
            ud_array = []
            up_array = []
            dn_array = []
            ...
            batting_average = float(
                df_forecast_five_day[df_forecast_five_day["ud"] == 1].shape[0] / df_forecast_five_day.shape[0])
            print("last 6 rows------------------------")
            ...
            change_factor = np.linspace(0, 3.20,
                                        len(change))  # now it's 93% 3/3.2 = 0.93 ~ 93% #71% means 3/4.1 = 0.71 ~ 71%
