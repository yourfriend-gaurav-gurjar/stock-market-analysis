...
def build_lstm_network(X_seq_length,
    ...
    model.add(LSTM(units=num_cell_units, batch_input_shape=(num_sequences_per_batch, X_seq_length, X_feature_length),
                   stateful=keep_state))
...
def forecast_lstm(model, X, n_batch):
    ...
    # make forecast
    forecast = model.predict_on_batch(X_f)
    # print(forecast)
    # convert to array
...
def sse_numba(symbol, look_back):
    ...
    X_train_close = np.asarray(X_train_close).astype(np.int)
    print("X_train_close------------------")
    print(len(X_train_close))
    print("y_train_close------------------")
    print(len(y_train_close))
    y_train_close = np.asarray(y_train_close).astype(np.int)
    ...
    if use_stateful_model:
        ...
        for i in range(NUM_EPOCHS_TRAINING):
            ...
            training_history.append(
                multistep_model_close.fit(X_train_close, y_train_close, epochs=1, batch_size=1, verbose=1,
                                          shuffle=False, use_multiprocessing=True, workers=8))
        ...
        training_history = multistep_model_close.fit(X_train_close, y_train_close, epochs=NUM_EPOCHS_TRAINING,
                                                     batch_size=0, validation_split=0.25, verbose=1, shuffle=False,
    ...
    print("last 5 rows------------------------")
    print(df_forecast_five_day.tail(5))
    result = df_forecast_five_day.tail(5)
    t4 = time.time()
...
symbol = 'AAPL'  # chnage stock symbol here if needed(AAPL = 'Apple')
# t2 = time.time()
look_back = 200
